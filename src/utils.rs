...
/// Given an unsigned integer ( byte length of customization string, which can be zero ),
/// this routine encodes it following section 3.3 of KangarooTwelve RFC 9861.
/// It returns an encoded byte array ( of length at max sizeof(usize) + 1 ) and effective length `len` <= sizeof(usize) + 1.
///
...
/// chunk ( s.t. each chunk is `CHUNK_BYTE_LEN` -bytes wide ) along with how many ( must be <= CHUNK_BYTE_LEN ) bytes
...
///
/// We can split S into `n` -chunks s.t. first (n - 1) chunks are of length B while the last one is of length <= B. So n = ⌈|S|/ B⌉
///
...
///
/// You may want to take a look at section 3.1 and 3.2 of the KangarooTwelve RFC for understanding why this function exists.
/// RFC 9861 https://www.rfc-editor.org/rfc/rfc9861.html.
#[inline(always)]
pub fn get_ith_chunk<const CHUNK_BYTE_LEN: usize>(i: usize, msg: &[u8], cstr: &[u8], enc: &[u8]) -> ([u8; CHUNK_BYTE_LEN], usize) {
    let l0 = msg.len();
    let l1 = l0 + cstr.len();
    let l2 = l1 + enc.len();

    let mut res = [0u8; CHUNK_BYTE_LEN];
    let mut off = 0;

    let start_at = i * CHUNK_BYTE_LEN;

    if start_at < l0 {
        let readable = cmp::min(l0 - start_at, CHUNK_BYTE_LEN);
        res[..readable].copy_from_slice(&msg[start_at..(start_at + readable)]);

        off += readable;
    }

    if (off < CHUNK_BYTE_LEN) && ((start_at + off) < l1) {
        let readable = cmp::min(l1 - (start_at + off), CHUNK_BYTE_LEN - off);
    ...
    if (off < CHUNK_BYTE_LEN) && ((start_at + off) < l2) {
        let readable = cmp::min(l2 - (start_at + off), CHUNK_BYTE_LEN - off);
